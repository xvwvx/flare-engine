	...
	, safe_video(false)
{
	...
	setConfigDefault(3,  "music_volume",        &typeid(music_volume),        "96",            &music_volume,        "Music and sound volume | 0 = silent, 128 = maximum");
	setConfigDefault(4,  "sound_volume",        &typeid(sound_volume),        "128",           &sound_volume,        "");
	setConfigDefault(5,  "combat_text",         &typeid(combat_text),         "1",             &combat_text,         "Display floating damage text | 0 = disable, 1 = enable");
	setConfigDefault(6,  "mouse_move",          &typeid(mouse_move),          "0",             &mouse_move,          "Use mouse to move | 0 = disable, 1 = enable");
	setConfigDefault(7,  "hwsurface",           &typeid(hwsurface),           "1",             &hwsurface,           "Hardware surfaces & V-sync. Try disabling for performance. | 0 = disable, 1 = enable");
	...
	setConfigDefault(10, "dpi_scaling",         &typeid(dpi_scaling),         "0",             &dpi_scaling,         "DPI-based render scaling | 0 = disable, 1 = enable");
	setConfigDefault(11, "parallax_layers",     &typeid(parallax_layers),     "1",             &parallax_layers,     "Rendering of parallax map layers | 0 = disable, 1 = enable");
	setConfigDefault(12, "max_fps",             &typeid(max_frames_per_sec),  "60",            &max_frames_per_sec,  "Maximum frames per second | 60 = default");
	setConfigDefault(13, "renderer",            &typeid(render_device_name),  "sdl_hardware",  &render_device_name,  "Default render device. | sdl_hardware = default, Try sdl for compatibility");
	setConfigDefault(14, "enable_joystick",     &typeid(enable_joystick),     "1",             &enable_joystick,     "Joystick settings.");
	setConfigDefault(15, "joystick_device",     &typeid(joystick_device),     "0",             &joystick_device,     "");
	setConfigDefault(16, "joystick_deadzone",   &typeid(joy_deadzone),        "8000",          &joy_deadzone,        "");
	setConfigDefault(17, "language",            &typeid(language),            "en",            &language,            "2-letter language code.");
	setConfigDefault(18, "change_gamma",        &typeid(change_gamma),        "0",             &change_gamma,        "Allow changing screen gamma (experimental) | 0 = disable, 1 = enable");
	setConfigDefault(19, "gamma",               &typeid(gamma),               "1.0",           &gamma,               "Screen gamma. Requires change_gamma=1 | 0.5 = darkest, 2.0 = lightest");
	setConfigDefault(20, "mouse_aim",           &typeid(mouse_aim),           "1",             &mouse_aim,           "Use mouse to aim | 0 = disable, 1 = enable");
	setConfigDefault(21, "no_mouse",            &typeid(no_mouse),            "0",             &no_mouse,            "Make using mouse secondary, give full control to keyboard | 0 = disable, 1 = enable");
	setConfigDefault(22, "show_fps",            &typeid(show_fps),            "0",             &show_fps,            "Show frames per second | 0 = disable, 1 = enable");
	setConfigDefault(23, "colorblind",          &typeid(colorblind),          "0",             &colorblind,          "Enable colorblind help text | 0 = disable, 1 = enable");
	setConfigDefault(24, "hardware_cursor",     &typeid(hardware_cursor),     "0",             &hardware_cursor,     "Use the system mouse cursor | 0 = disable, 1 = enable");
	setConfigDefault(25, "dev_mode",            &typeid(dev_mode),            "0",             &dev_mode,            "Developer mode | 0 = disable, 1 = enable");
	setConfigDefault(26, "dev_hud",             &typeid(dev_hud),             "1",             &dev_hud,             "Show additional information on-screen when dev_mode=1 | 0 = disable, 1 = enable");
	setConfigDefault(27, "loot_tooltips",       &typeid(loot_tooltips),       "0",             &loot_tooltips,       "Loot tooltip mode | 0 = normal, 1 = show all, 2 = hide all");
	setConfigDefault(28, "statbar_labels",      &typeid(statbar_labels),      "0",             &statbar_labels,      "Always show labels on HP/MP/XP bars | 0 = disable, 1 = enable");
	setConfigDefault(29, "statbar_autohide",    &typeid(statbar_autohide),    "1",             &statbar_autohide,    "Allow the HP/MP/XP bars to auto-hide on inactivity | 0 = disable, 1 = enable");
	setConfigDefault(30, "auto_equip",          &typeid(auto_equip),          "1",             &auto_equip,          "Automatically equip items | 0 = disable, 1 = enable");
	setConfigDefault(31, "subtitles",           &typeid(subtitles),           "0",             &subtitles,           "Subtitles | 0 = disable, 1 = enable");
